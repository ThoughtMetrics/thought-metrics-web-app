// src/shared/ui/atoms/survey-questions/VideoUpload.tsx
...
export interface VideoUploadProps extends BaseSurveyQuestionProps {
  questionId: string;
  value?: UploadedFile;
  ...
  const validateFile = (file: File): Promise<string | null> => {
    return new Promise((resolve) => {
      ...
      if (!ALLOWED_VIDEO_TYPES.includes(file.type)) {
        resolve('Only MP4, WebM, or MOV video files are supported');
        return;
      }
      ...
      videoEl.onloadedmetadata = () => {
        ...
        if (videoEl.duration > maxDurationSec) {
          resolve(`Video must be under ${maxDurationSec} seconds`);
        } else {
          resolve(null);
        }
      };
      videoEl.onerror = () => { URL.revokeObjectURL(videoEl.src); resolve(null); };
