        ...
        {items.map((item) => (
          <div key={item.id} className="space-y-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-sm md:text-base text-custom-grey-3">
                {selectedValues[item.id] ?? minValue}
              </span>
            </div>

            {/* Min/Max Labels */}
            <div className="flex justify-between text-xs md:text-sm text-custom-grey-3">
              <span className="text-primary">{item.minLabel}</span>
              <span className="text-primary">{item.maxLabel}</span>
            </div>
            ...
            <input
              ...
              min={minValue}
              max={maxValue}
              value={selectedValues[item.id] ?? minValue}
              onChange={(e) =>
              ...
              style={{
                background: `linear-gradient(to right, #E63946 0%, #E63946 ${((selectedValues[item.id] ?? minValue) - minValue) / (maxValue - minValue) * 100}%, #E5E5E5 ${((selectedValues[item.id] ?? minValue) - minValue) / (maxValue - minValue) * 100}%, #E5E5E5 100%)`,
              }}
            />
          </div>
        ))}
