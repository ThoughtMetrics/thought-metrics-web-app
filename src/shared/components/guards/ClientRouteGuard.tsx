import React, { useEffect, useState } from 'react';
import { QueryClientProvider } from '@tanstack/react-query';
import { queryClient } from '@/core/lib/query-client';
import { AuthProvider, useAuth } from '@/shared/providers/auth-provider';
import { LoaderUI } from '@/shared/ui/atoms/loader/LoaderUI';

interface ClientRouteGuardProps {
  children: React.ReactNode;
}

const ClientRouteGuardInternal: React.FC<ClientRouteGuardProps> = ({ children }) => {
  const { user, isAuthReady, isClient } = useAuth();
  const [isChecking, setIsChecking] = useState(true);

  const isLocalhost = typeof window !== 'undefined' &&
    (window.location.hostname === 'localhost' ||
     window.location.hostname === '127.0.0.1' ||
     window.location.hostname.includes('192.168.'));

  useEffect(() => {
    if (!isAuthReady) {
      console.debug('[ClientRouteGuard] Waiting for auth to be ready...');
      return;
    }

    if (isLocalhost) {
      setIsChecking(false);
      return;
    }

    if (!user) {
      console.debug('[ClientRouteGuard] User not authenticated, redirecting to login');
      window.location.href = '/login?redirect=' + encodeURIComponent(window.location.pathname);
      return;
    }

    if (!isClient) {
      console.debug('[ClientRouteGuard] User lacks client role, redirecting to unauthorized');
      window.location.href = '/unauthorized';
      return;
    }

    console.debug('[ClientRouteGuard] Access granted, rendering content');
    setIsChecking(false);
  }, [user, isAuthReady, isClient, isLocalhost]);

  if (!isAuthReady || isChecking) {
    return (
      <div className="h-full flex items-center justify-center bg-gray-50">
        <LoaderUI message="Verifying access..." />
      </div>
    );
  }

  return <>{children}</>;
};

export const ClientRouteGuard: React.FC<ClientRouteGuardProps> = ({ children }) => {
  return (
    <QueryClientProvider client={queryClient}>
      <AuthProvider>
        <ClientRouteGuardInternal>
          {children}
        </ClientRouteGuardInternal>
      </AuthProvider>
    </QueryClientProvider>
  );
};

export default ClientRouteGuard;
