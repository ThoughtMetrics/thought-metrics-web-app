    ...
    <div className={`flex items-center gap-3 px-2 py-3 ${collapsed ? 'justify-center' : ''}`}>
      ...
      {!collapsed && (
        <div className="min-w-0">
          <p className="text-xs font-medium text-gray-700 leading-tight">Storage</p>
...
const ClientSidebar: React.FC = () => {
  ...
  return (
    <aside className={`${isCollapsed ? 'w-14' : 'w-64'} bg-white border-r border-gray-200 min-h-screen flex-shrink-0 transition-all duration-200 flex flex-col`}>
      <div className={`p-3 ${isCollapsed ? '' : 'px-6'} flex-1`}>
        <div className="flex items-center justify-between mb-6">
          {!isCollapsed && (
            <h2 className="text-2xl font-bold text-gray-800">Client Panel</h2>
          )}
          <button
