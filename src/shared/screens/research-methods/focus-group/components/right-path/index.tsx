import { GreyEllipseIcon, RedEllipseIcon } from '@/assets';
import { cn } from '@/core/utils/cn';

const RightPathSection: React.FC<any> = ({
  rightPathSection,
  className,
}: any) => {
  return (
    <section className="common-component relative bg-primary-lighter text-black">
      <div className="absolute top-0 -right-8 md:right-0 transform -translate-y-1/2 z-1">
        <RedEllipseIcon className="h-32 md:h-60 xxl:h-[35rem]" />
      </div>
      <div className="absolute bottom-0 -left-10 md:-left-4 transform translate-y-1/2 z-1">
        <GreyEllipseIcon className="h-32 md:h-60 xxl:h-[35rem]" />
      </div>
      <div
        className={cn(
          'common-container px-6 py-8 md:px-24 md:py-24 max-w-[1336px]! flex-col',
          className
        )}
      >
        <h2 className="w-[80%] md:w-[20%] xxl:w-[30%] text-xl xl:text-2xl xxl:text-3xl font-semibold leading-tight">
          <span>{rightPathSection.title}</span>
        </h2>
        <img
          src="/illustrations/illustration-right-respondents.svg"
          alt="Right respondents illustration"
          className="hidden md:block w-full h-fit"
        />
        <img
          src="/illustrations/illustration-right-respondents-m.svg"
          alt="Right respondents illustration mobile"
          className="md:hidden w-full h-fit"
        />
      </div>
    </section>
  );
};

export default RightPathSection;
