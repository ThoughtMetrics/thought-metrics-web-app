...
const SurveyDetailSection: React.FC<SurveyDetailSectionProps> = ({
  ...
  const [isFromTrackingLink] = useState(
    () => typeof window !== 'undefined' && !!localStorage.getItem('tm_link_id')
  );
  ...
  const visibleIndices = React.useMemo(() => {
    ...
    return allQs
      ...
      .filter((i) => {
        ...
        if (
          cfg.showIfAny?.length &&
          !cfg.showIfAny.some((c: any) => evalCond(c))
        )
          return false;
        return true;
      });
  ...
  const getFilteredOptions = React.useCallback(
    (
      ...
      const optionFilter = cfg.optionFilter as
        | { questionId: string; map: Record<string, string[]> }
        | undefined;
  ...
  const getSurveyLabel = () => {
    ...
    return (
      translatedLabel ||
      template.translations?.en?.label ||
      template.label ||
      ''
    );
  };
  ...
  React.useEffect(() => {
    ...
    qs.forEach((q, idx) => {
      ...
      // Q type: Zone — autoPopulateFrom user's assigned zone (static options in config)
      if (cfg.autoPopulateFrom === 'user.zonalInfo.zone') {
        const zonalList = ((userProfile as any).zonalInfo || []) as any[];
        const userZone = zonalList[0]?.zone as string | undefined;
        // config.options already language-localized by the survey-details API
        const opts = (q.config?.options || []).map((opt: any) => ({
        ...
        if (userZone) {
          setAnswers((prev) => {
            ...
            return { ...prev, [idx]: { value: userZone } };
          });
        }
      }

      // Q type: AC Name — autoPopulateFrom user's assigned AC
      if (cfg.autoPopulateFrom === 'user.zonalInfo.assemblyConstituency') {
        const zonalList = ((userProfile as any).zonalInfo || []) as any[];
        if (zonalList.length === 1) {
          const ac = zonalList[0];
          const label =
            ac.translations?.[language]?.assemblyConstituency ||
            ac.assemblyConstituency;
          // Always update dynamic options (language may have changed)
          setDynamicOptions((prev) => ({
            ...prev,
            [q.id]: [{ label, value: String(ac.acNo) }],
          }));
          // Always update displayLabel so it reflects the current language;
          // preserve existing value (do not overwrite a draft answer)
          setAnswers((prev) => ({
            ...prev,
            [idx]: {
              ...(prev[idx] || {}),
              value: prev[idx]?.value || String(ac.acNo),
              displayLabel: label,
            },
          }));
  ...
  React.useEffect(() => {
    ...
    qs.forEach((q) => {
      ...
      const parentValue =
        parentIdx !== undefined ? answers[parentIdx]?.value : undefined;
      if (!parentValue) return;

      const parentValueStr = String(parentValue);
      // Skip if neither the parent value nor the language has changed since last fetch
      const cacheKey = `${parentValueStr}::${language}`;
      if (prevParentValues.current[q.id] === cacheKey) return;
      prevParentValues.current[q.id] = cacheKey;
      ...
      if (cfg.dataSource === 'zones:villages') {
        ...
        const grandParentIdx =
          parentChain.length > 0
            ? questionIdToIndex[parentChain[0]]
            : undefined;
        const grandParentValue =
          grandParentIdx !== undefined
            ? answers[grandParentIdx]?.value
            : undefined;
        ...
        setLoadingOptions((prev) => ({ ...prev, [q.id]: true }));
        zoneService
          .getVillages(acNo, parentValueStr)
          .then((villages) => {
  ...
  const isQuestionValid = React.useCallback(
      ...
      switch (qData.questionType) {
        ...
        case QuestionType.MULTI_SLIDER: {
          ...
          return (
            answer.values &&
            sliders.length > 0 &&
            sliders.every((slider: any) => {
              const sliderId = slider.id ?? slider.value;
              return (
                answer.values[sliderId] !== undefined &&
                answer.values[sliderId] !== null
              );
            })
          );
        }
    ...
    [surveyData, answers, visibleIndices, language]
  ...
  const visibleQuestionNumber =
    visibleIndices.indexOf(currentQuestion) + 1 || 1;
  ...
  const handleAnswerChange = (answer: any) => {
    ...
    if (currentQ) {
      questions.forEach((q, idx) => {
        ...
        const isInParentChain = (q.config?.parentChain || []).includes(
          currentQ.id
        );
  ...
  const handleSubmit = async () => {
    ...
    if (userProfile) {
      ...
      if (userProfile.profile?.phone)
        respondent.phone = userProfile.profile.phone;
    }
    ...
    const onError = (error: any) => {
      toast.error(
        error?.details?.error?.message ||
          translations.toast.surveySubmittedError
      );
    };
  ...
  const renderQuestion = (
    ...
    const commonProps = {
      ...
      onCommentChange: (comment: string) =>
        onChange({ ...currentAnswer, comment }),
    ...
    switch (qData.questionType) {
      case QuestionType.LIKERT_SCALE:
        return (
          <LickertScale
            ...
            minLabel={
              config.minLabel ||
              (config.labels ? (Object.values(config.labels)[0] as string) : '')
            }
            maxLabel={
              config.maxLabel ||
              (config.labels ? (Object.values(config.labels)[1] as string) : '')
            }
      ...
      case QuestionType.MCQ_SINGLE: {
        ...
        const rawOptions = hasDynamicSource
          ? rawDynOpts
        ...
        if (isReadOnly) {
          const displayLabel =
            currentAnswer?.displayLabel ||
            mcqSingleOptions.find((o: any) => o.value === currentAnswer?.value)?.label ||
            currentAnswer?.value ||
            '';
          return (
            ...
            minLabel={
              config.labels
                ? Array.isArray(config.labels)
                  ? config.labels[0]
                  : (config.labels.start ?? config.labels.min ?? '')
                : ''
            }
            maxLabel={
              config.labels
                ? Array.isArray(config.labels)
                  ? config.labels[1]
                  : (config.labels.end ?? config.labels.max ?? '')
                : ''
            }
            step={config.step}
      ...
      case QuestionType.MULTI_SLIDER: {
        const sliderItems = (config.items || config.sliders || []).map(
          (slider: any) => ({
            ...slider,
      ...
      case QuestionType.RANKING: {
        const rankingItems = (config.options || config.items || []).map(
          (item: any) => ({
            ...item,
        ...
        return (
          <MaxDiff
            ...
            onSelectionChange={(selections) =>
              onChange({ ...currentAnswer, selections })
            }
          />
        );
      }
      ...
      case QuestionType.CONSTANT_SUM: {
        const constantSumOptions = (config.options || config.items || []).map(
          (item: any) => ({
            ...item,
            id: item.id ?? item.value,
            value: item.value ?? item.id,
        ...
        return (
          <SurveyQuestionWrapper {...commonProps}>
            <input
              ...
              placeholder={
                qData.translations?.[language]?.placeholder ||
                config.placeholder ||
                ''
              }
  ...
  return (
    <SurveyLayoutContext.Provider
      value={{ layout: formLayout, setLayout: setFormLayout }}
    >
      {formLayout === 'list' ? (
        renderListLayout()
      ) : (
