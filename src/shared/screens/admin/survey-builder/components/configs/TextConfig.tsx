...
const TextConfig: React.FC<Props> = ({ question, qIdx, lang }) => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <label className="block text-xs font-medium text-on-surface mb-1">Placeholder</label>
        <input
          type="text"
          value={t.placeholder ?? ''}
      ...
      {(isText || isTextarea) && lang === 'en' && (
        <div className="space-y-3 pt-1 border-t border-outline-variant/50">
          <span className="block text-xs font-semibold text-outline uppercase tracking-wide">Character Limits</span>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-on-surface mb-1">Min chars</label>
              <input
                type="number"
                min={0}
                value={cfg.minChars ?? ''}
            ...
            <div>
              <label className="block text-xs font-medium text-on-surface mb-1">Max chars</label>
              <input
                type="number"
                min={1}
                value={cfg.maxChars ?? ''}
          ...
          {cfg.maxChars && (
            <p className="text-xs text-outline">Character counter will be shown to respondents.</p>
          )}
        </div>
      )}
      ...
      {(isText || isTextarea) && lang === 'en' && (
        <div className="space-y-3 pt-1 border-t border-outline-variant/50">
          ...
          {isText && (
            <div>
              <label className="block text-xs font-medium text-on-surface mb-1">Width (px)</label>
              <input
                type="number"
                min={50}
          ...
          {isTextarea && (
            <div>
              <label className="block text-xs font-medium text-on-surface mb-1">Height (px)</label>
              <input
                type="number"
                min={60}
      ...
      {isNumber && lang === 'en' && (
        <div className="space-y-3 pt-2 border-t border-outline-variant/50">
          <div className="flex items-center justify-between py-2 px-3 bg-surface-container-low border border-outline-variant rounded-lg">
            <span className="text-xs font-medium text-on-surface-variant">Allow "Don't Know / Refuse"</span>
            <ToggleSwitch
          ...
          {cfg.hasDontKnow && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-on-surface mb-1">Button label</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-medium text-on-surface mb-1">Stored value</label>
                <input
                  type="text"
        ...
        <div className="space-y-2 pt-1 border-t border-outline-variant/50">
          ...
          <div className="flex gap-4">
            {(['period', 'comma'] as const).map((d) => (
              <label key={d} className="flex items-center gap-1.5 cursor-pointer text-xs text-on-surface">
                <input
                  type="radio"
                  name={`decimal-${qIdx}`}
      ...
      {question.questionType === QuestionType.CURRENCY && (
        <div>
          <label className="block text-xs font-medium text-on-surface mb-1">Currency Code</label>
          <input
            type="text"
            value={cfg.currency ?? ''}
