...
const FileConfig: React.FC<Props> = ({ question }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 border-2 border-dashed border-outline-variant rounded-xl px-6 py-8 text-center select-none pointer-events-none">
      ...
      <span className="text-sm text-outline">Drop files here or click to browse</span>
      <div className="flex flex-col items-center gap-0.5">
        {accepted.length > 0 && (
          <span className="text-xs text-outline/40 uppercase tracking-wide">{accepted.join(' · ')}</span>
        )}
        <span className="text-xs text-outline/40">Max {maxMb} MB</span>
      </div>
    </div>
  );
};
