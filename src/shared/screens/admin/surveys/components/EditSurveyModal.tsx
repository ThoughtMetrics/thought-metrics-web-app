// src/shared/screens/admin/surveys/components/EditSurveyModal.tsx
...
import type { ISurveyUpdateRequest } from '@/core/types/survey-builder.type';
...
const EditSurveyModal: React.FC<Props> = ({ survey, onClose }) => {
  const updateSurvey = useUpdateSurveyInstance();

  const [label, setLabel] = useState(survey.label);
  const [type, setType] = useState<'respondent' | 'agent'>((survey.type as any) ?? 'respondent');
  const [visibility, setVisibility] = useState<'public' | 'private'>((survey.visibility as any) ?? 'public');
  const [startDate, setStartDate] = useState(survey.startDate ? survey.startDate.slice(0, 10) : '');
  const [expireDate, setExpireDate] = useState(survey.expireDate ? survey.expireDate.slice(0, 10) : '');
  const [maxResponses, setMaxResponses] = useState(survey.maxResponses ? String(survey.maxResponses) : '');
  const [zonalBasedSurvey, setZonalBasedSurvey] = useState(survey.zonalBasedSurvey ?? false);
  const [formLayout, setFormLayout] = useState<'paginated' | 'list'>((survey.formLayout as any) ?? 'paginated');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: ISurveyUpdateRequest = {
      label: label.trim(),
      type,
      visibility,
    ...
    await updateSurvey.mutateAsync({ id: survey.id, data: payload });
    onClose();
  };
