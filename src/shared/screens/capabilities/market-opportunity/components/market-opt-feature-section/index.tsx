              ...
              {subitem.map((feature: any, colIndex: number) => (
                <div className="relative w-full h-full z-1 md:px-10 md:py-6">
                  ...
                  {colIndex !== subitem.length - 1 && (
                    <line className="absolute w-28 bg-black h-px -right-18 top-1/2" />
                  )}
                </div>
              ))}
              {rowIndex !== featureSection.items.length - 1 && (
                <line className="absolute w-[65%] md:w-[95%] bg-black h-px left-1/2 -bottom-7.5 md:-bottom-3 transform -translate-x-1/2" />
              )}
              {rowIndex !== featureSection.items.length - 1 && (
                <div className="hidden md:block absolute -bottom-[.78rem] right-0">
                  <div className="relative w-10 h-16.5 rounded-tr-xl rounded-br-xl border-y border-r border-black"></div>
                </div>
              )}
              {rowIndex !== 0 && (
                <div className="hidden md:block absolute -top-[.78rem] left-0">
                  <div className="relative w-11.5 h-16.5 border-black border-y border-l rounded-tl-xl rounded-bl-xl"></div>
                </div>
              )}
            </div>
          ))}
